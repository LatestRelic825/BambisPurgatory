...
class PlayState extends MusicBeatState
{
	...
	//goofy characters
	var funnyFloatyBoys:Array<String> = ['dave-3d', 'bambi-3d', 'crusti', 'baiburg', 'crusturn', 'minion', 'god-expunged-1', 'bambi-unfair', 'expunged', 'bambi-piss-3d', 'bambi-scaryooo', 'bambi-god', 'bambi-god-24fps', 'bambi-god2d', 'bambi-god-2-24fps', 'bambi-hell', 'bombureal', 'bombai', 'crimson-dave', 'crimson-bambi', 'gary', 'bamburg', 'bamburg-player'];
	...
	/*	var ext = 100;
		...
		case 'farmNight':
			...
			case 'pcworld':
				...
				pcworld = new FlxSprite(-600, -200).loadGraphic(Paths.image('bpASSets/bombu/pcworld'));
			...
			case 'burger':
				...
				burger = new FlxSprite(-600, -200).loadGraphic(Paths.image('bpASSets/bamburg/hamburger'));
				...
				//	}
					...
					/*if(SONG.song.toLowerCase() == 'upheaval') {
						...
						if (!ClientPrefs.lowQuality) {
							olddavesky = new FlxSprite(-600, -200).loadGraphic(Paths.image('dave/oldass/sky'));
							olddavesky.antialiasing = true;
							olddavesky.scrollFactor.set(0.9, 0.9);
							olddavesky.active = false;
							add(olddavesky);
			
							olddaveHills = new FlxSprite(-225, -125).loadGraphic(Paths.image('dave/oldass/hills'));
							olddaveHills.setGraphicSize(Std.int(olddaveHills.width * 1.25));
							olddaveHills.updateHitbox();
							olddaveHills.antialiasing = true;
							olddaveHills.scrollFactor.set(1, 1);
							olddaveHills.active = false;
							add(olddaveHills);
			
							olddaveGate = new FlxSprite(-225, -125).loadGraphic(Paths.image('dave/oldass/gate'));
							olddaveGate.setGraphicSize(Std.int(olddaveGate.width * 1.2));
							olddaveGate.updateHitbox();
							olddaveGate.antialiasing = true;
							olddaveGate.scrollFactor.set(0.925, 0.925);
							olddaveGate.active = false;
							add(olddaveGate);
			
							olddaveGrass = new FlxSprite(-225, -125).loadGraphic(Paths.image('dave/oldass/grass'));
							olddaveGrass.setGraphicSize(Std.int(olddaveGrass.width * 1.2));
							olddaveGrass.updateHitbox();
							olddaveGrass.antialiasing = true;
							olddaveGrass.scrollFactor.set(0.9, 0.9);
		...
		{
			case 'bambi-god' | 'bambi-god2d' | 'bambi-god-2-24fps' | 'expunged':
	...
	{
		elapsedtime += elapsed;
		//Time elapsed since last frame or some shit like that
		...
		if(canFloat && !funnyFloatyBoys.contains(boyfriend.curCharacter.toLowerCase())) {
			switch (curStage) {
				...
				//	redPlatform.y += (Math.sin(elapsedtime) * 0.3);
				}
			}
		}
		//shit for shaders startin here
		if (curbg != null) {
			if (curbg.active) {
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		for (i in seperatedScore)
		{
			...
			if(ClientPrefs.ratingsinHUD) 
				if(comboSpr.x > xThing) xThing = comboSpr.x;
		}
