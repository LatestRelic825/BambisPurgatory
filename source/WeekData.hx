...
class WeekData {
	...
	//since i dont want to copy paste every freeplay to make every category song im making this
	public static function reloadCustomWeekFiles(customWeek:String, isStoryMode:Null<Bool> = false) 
		{
			weeksList = [];
			weeksLoaded.clear();
			#if MODS_ALLOWED
			...
			var sexList:Array<String> = CoolUtil.coolTextFile(Paths.getPreloadPath('weeks/${customWeek}/weekList.txt'));
			for (i in 0...sexList.length) {
				for (j in 0...directories.length) {
					var fileToCheck:String = directories[j] + 'weeks/${customWeek}/${sexList[i]}.json';
			...
			for (i in 0...directories.length) {
				var directory:String = directories[i] + 'weeks/${customWeek}/';
